...
namespace s3iLib
{
    public class AmazonS3ClientMap
    {
        ...
        public static Lazy<RegionEndpoint> DefaultRegion { get; set; } = new Lazy<RegionEndpoint>(() =>
        {
            return Amazon.RegionEndpoint.GetBySystemName(AmazonAccount.RegionName);
        });
        public AmazonS3ClientMap(AWSCredentials credentials, AmazonS3Client client = null)
        {
            Credentials = credentials;
            Client = client ?? new AmazonS3Client(Credentials, DefaultRegion?.Value);
        ...
        public async Task<AmazonS3Client> GetClientAsync(string bucketName)
        {
            if (null == Client)
            {
                Client = new AmazonS3Client(Credentials, DefaultRegion?.Value);
                region2client[Client.Config.RegionEndpoint.SystemName] = Client;
            }
            if (!bucket2region.TryGetValue(bucketName, out var bucketLocation))
            {
                ...
                {
                    Console.WriteLine($"? {nameof(AmazonS3ClientMap)}.{nameof(GetClientAsync)}({bucketName}[{DefaultRegion?.Value?.SystemName}]): {x.GetType().Name}");
                    throw;
                }
            }
